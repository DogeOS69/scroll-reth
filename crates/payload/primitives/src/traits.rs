...
impl PayloadAttributes for scroll_alloy_rpc_types_engine::ScrollPayloadAttributes {
    fn payload_id(&self, parent_hash: &B256) -> PayloadId {
        ...
        if self.no_tx_pool || self.transactions.as_ref().is_some_and(|txs| !txs.is_empty()) {
            hasher.update([self.no_tx_pool as u8]);
        ...
        if let Some(extra_data) = &self.block_data_hint.extra_data {
            hasher.update(extra_data);
        }
        if let Some(state_root) = &self.block_data_hint.state_root {
            hasher.update(state_root.0);
        }
        if let Some(coinbase) = &self.block_data_hint.coinbase {
            hasher.update(coinbase);
        }
        if let Some(nonce) = &self.block_data_hint.nonce {
            hasher.update(nonce.to_be_bytes());
        }
        if let Some(difficulty) = self.block_data_hint.difficulty {
