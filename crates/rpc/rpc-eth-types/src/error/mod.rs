...
use revm::database_interface::bal::EvmDatabaseError;
    ...
    E: Into<Self>,
{
    fn from(value: EvmDatabaseError<E>) -> Self {
        match value {
            EvmDatabaseError::Bal(err) => Self::Internal(RethError::msg(err)),
...
pub fn ensure_success<Halt, Error: FromEvmHalt<Halt> + FromEthApiError>(
    ...
    match result {
        ...
        ExecutionResult::Halt { reason, gas, .. } => {
            Err(Error::from_evm_halt(reason, gas.tx_gas_used()))
        }
    }
}
