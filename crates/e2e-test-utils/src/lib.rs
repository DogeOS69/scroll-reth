...
/// Type alias for the default RPC handle for a temporary node adapter.
pub type TmpRpcHandle<N> = reth_node_builder::rpc::RpcHandle<Adapter<N>, TmpNodeEthApi<N>>;
...
where
    Self: Default
        + NodeTypesForProvider<Payload: PayloadTypes>
        + Node<
