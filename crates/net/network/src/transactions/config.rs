...
use alloy_consensus::{EthereumTxEnvelope, TxType};
...
/// Static transaction type support for pooled transaction envelopes.
pub trait SupportedTxTypes {
    /// Returns true if the given EIP-2718 type byte is supported by this transaction envelope.
    fn supports_tx_type(ty: u8) -> bool;
}

impl<Eip4844> SupportedTxTypes for EthereumTxEnvelope<Eip4844> {
    fn supports_tx_type(ty: u8) -> bool {
        TxType::try_from(ty).is_ok()
    }
}
...
impl<N> AnnouncementFilteringPolicy<N> for TypedStrictFilter
where
    N: NetworkPrimitives,
    N::PooledTransaction: SupportedTxTypes,
{
    fn decide_on_announcement(&self, ty: u8, hash: &B256, size: usize) -> AnnouncementAcceptance {
        if N::PooledTransaction::supports_tx_type(ty) {
...
impl<N> AnnouncementFilteringPolicy<N> for TypedRelaxedFilter
where
    N: NetworkPrimitives,
    N::PooledTransaction: SupportedTxTypes,
{
