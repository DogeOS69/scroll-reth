...
impl PayloadAttributes for scroll_alloy_rpc_types_engine::ScrollPayloadAttributes {
    fn payload_id(&self, parent_hash: &B256) -> PayloadId {
        scroll_payload_id(parent_hash, self)
    }
...
/// Generates a Scroll payload ID from all fields that can affect the resulting block.
#[cfg(feature = "scroll-alloy-traits")]
fn scroll_payload_id(
    parent: &B256,
    ...
    if attributes.no_tx_pool ||
        attributes.transactions.as_ref().is_some_and(|transactions| !transactions.is_empty())
    {
        hasher.update([attributes.no_tx_pool as u8]);
        let transactions_len = attributes.transactions.as_ref().map(Vec::len).unwrap_or_default();
        hasher.update(transactions_len.to_be_bytes());
    ...
    if let Some(extra_data) = &hint.extra_data {
        hasher.update(extra_data);
    }
    if let Some(state_root) = hint.state_root {
        hasher.update(state_root);
    }
    if let Some(coinbase) = hint.coinbase {
        hasher.update(coinbase);
    }
    if let Some(nonce) = hint.nonce {
        hasher.update(nonce.to_be_bytes());
    }
    if let Some(difficulty) = hint.difficulty {
